...
class CallBackVerification(object):
    ...
    def ver_test(self, backbone: torch.nn.Module, global_step: int):
        ...
        for i in range(len(self.ver_list)):
            acc, embeddings_list = verification.test_image_dataloader(
                self.ver_list[i], backbone)
            logging.info('[%s][%d]' % (self.ver_name_list[i], global_step))
            logging.info('[%s][%d]Accuracy-Flip: %1.5f' % (self.ver_name_list[i], global_step, acc))

            self.summary_writer: SummaryWriter
            self.summary_writer.add_scalar(tag=self.ver_name_list[i], scalar_value=acc, global_step=global_step, )
    ...
    def init_dataset(self, val_targets, data_dir, image_size):
        for path in val_targets:
            ...
            if os.path.exists(path):
                ...
                data_loader = verification.load_image_folder(path, image_size) #set dataset as dataloader
