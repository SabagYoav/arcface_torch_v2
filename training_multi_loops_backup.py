...
ROI_RATIOS = [1.0, 0.8, 0.6, 0.5 ,0.4, 0.35, 0.3, 0.25, 0.2]
ROI_WIDTH_RATIO = 0.8        # relative to face width
...
def read_and_update_variant_config(tag:str, ff_dir, pf_dir):
    train_dir = VARIANTS_DATASET_ROOT / "train"
    val_targets = [str(VARIANTS_DATASET_ROOT / "val"), str(train_dir)]
    exp_name = f"arcface_{tag}"
...
def write_variant_dataset(ratio):
    ...
    for split in ["train", "val"]:
        write_split(
            ...
            dst_root=VARIANTS_DATASET_ROOT / split,
            roi_ratio=ratio,
            roi_w_ratio=ROI_WIDTH_RATIO
        )
        save_split_sample(dst_root=ROI_ROOT / tag / split, split=split)
...
def main():
    ...
    for ratio in ROI_RATIOS:
        ...
        args = read_and_update_variant_config(tag, ff_dir = FULLFACE_ROOT, pf_dir = VARIANTS_DATASET_ROOT )
